...
export const gameStart = async (uuid, payload) => {
  ...
  const { stages, itemUnlocks } = await getGameAssets();
  // 이전 스테이지 비우기
  ...
  // 콘솔 확인
  const stageData = await getStage(uuid);
  console.log('Stage:', stageData);
  // 첫번째 스테이지 아이템 해금
  await setUnlockedItems(uuid, stages.data[0].id, itemUnlocks);
...
export const gameEnd = async (uuid, payload) => {
  ...
  const gotItems = await getGotItems(uuid);

  if (!stages || !stages.length) {
    return { status: 'fail', message: 'No stages found for user' };
  }

  const { items } = await getGameAssets();
